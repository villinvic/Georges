...
class Policy(tf.keras.Model):
    ...
    def __init__(self, action_dim, epsilon, layer_dims, lstm_dim, default_activation='elu',
                 name="CategoricalActor"):
        super().__init__(name=name)
        self.dist = Categorical(dim=action_dim)
        self.action_dim = action_dim
    ...
    def _compute_feature(self, states):
        features, self.h = self.lstm(states, initial_state=self.h)
        for layer in self.denses:
            features = layer(features)
        return features
...
class AC(tf.keras.Model, Default):
    ...
    def train(self, states, actions, rewards, probs, gpu):
        ...
        v_loss, total_loss, mean_entropy, min_entropy, max_entropy, min_logp, max_logp, grad_norm \
            = self._train(states, actions, rewards, probs, gpu)
    ...
    @tf.function
    def _train(self, states, actions, rewards, probs, gpu):
        '''
        ...
        with tf.device("/gpu:{}".format(gpu) if gpu >= 0 else "/cpu:0"):
            ...
            with tf.GradientTape() as tape:
                ...
                lstm_states = self.lstm(states)

                v_all = self.V(lstm_states)
                v = v_all[:, :-1, 0]
                last_v = v_all[:, -1, 0]
